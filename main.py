...
class App(customtkinter.CTk):
    ...
    def main_menu(self):
        ...
        self.button_salik = customtkinter.CTkButton(master=self.frame_left, text="Salik", height=32, command=self.salik_menu)
        self.button_salik.pack(pady=10, padx=10)

        self.button_departure = customtkinter.CTkButton(master=self.frame_left, text="Departure", height=32, command=self.departure_menu)
    ...
    def transaction_menu(self):
        def transactions(self):
            ...
            if len(transactions) == 0:
                ...
                self.scroll_canvas.bind_all("<MouseWheel>", lambda x: _on_mousewheel(self, x))
    ...
    def departure_menu(self):
        def departure_display(self):
            ...
            self.entry_departure = customtkinter.CTkEntry(master=self.frame_dropdown, width=400, placeholder_text="Enter departure")
            self.entry_departure.grid(row=0, column=0, pady=5, padx=5)

            self.button_search = customtkinter.CTkButton(master=self.frame_right, text="Search", width=120, command=lambda: view_results(self))
        ...
        def view_results(self):
            ...
            for i in self.deps:
                ...
                self.train_icon = customtkinter.CTkButton(
                    master=self.icon_frame,
                    text="",
                    state="diabled",
                    fg_color=f"{self.color_1}",
                    image=self.load_image(r"\assets\images\dep_icon.png", 40, 40),
                    width=40,
                    height=40,
                )
                self.train_icon.grid(row=0, column=0, pady=0, padx=0)

                self.mode = customtkinter.CTkButton(
                    master=self.icon_frame,
                    text=i["mode"],
                    text_color="#ffffff",
    ...
    def settings_menu(self):
        ...
        def change_appearance_mode(self, value):
            ...
            if value == "Dark":
                ...
                self.color_2 = "#ebebeb"
                self.color_3 = "#c8c7c7"
                self.iconphoto(True, tkinter.PhotoImage(file=PATH + r"\assets\images\logo_dark.png"))
